document.addEventListener('DOMContentLoaded', () => {
	...
	menuLinks.forEach(a => {
		a.addEventListener('click', () => {
			// data-target이 있으면 해당 패널의 열림 상태를 기준으로 토글
			const target = a.getAttribute('data-target');
			if (target) {
				const panel = document.querySelector(`.left-panel[data-panel="${target}"]`);
				const opened = !!(panel && panel.classList.contains('show'));
				if (opened) {
					clearActive();
					a.classList.add('is-active');   // 패널이 열린 상태면 활성 표시
				} else {
					a.classList.remove('is-active'); // 패널을 닫았다면 활성 해제
				}
			} else {
				// data-target이 없는 일반 항목은 단순 단일 선택으로 표시
				clearActive();
	...
	// 사이드바가 닫히면 활성 표시도 함께 해제 (기존 코드 건드리지 않고 감지만 함)
	if (sideBar) {
		new MutationObserver(() => {
			if (!sideBar.classList.contains('active')) clearActive();
