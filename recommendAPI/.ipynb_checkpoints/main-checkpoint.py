from fastapi import FastAPI, Body # FastAPI : 인공지능 서버를 별도로 구축해서 서비스에 통합할 수 있는 백엔드 프레임워크 
...
from models import LogsAndFeeds, FeedForSearch, ReviewFeed # 클래스들이 모여있는 파일
...
# --- 추천 음식점 idx 리스트 반환 ---
@app.post("/receive_res", response_model=List[int])
async def receive_res(query: str = Body(..., media_type="text/plain")):
